    ...
    <div style={{
      ...
      <div style={{ marginBottom: 32 }}>
        <p style={{
          ...
          letterSpacing: '0.08em', textTransform: 'lowercase', marginBottom: 10,
        }}>before you go</p>
        <h2 style={{
          fontFamily: 'var(--font-nunito), sans-serif',
          fontSize: 22, fontWeight: 600, color: 'var(--text-primary)', lineHeight: 1.3,
        }}>
          what would you like<br />to hold from tonight?
        </h2>
        <p style={{
          fontFamily: 'var(--font-nunito), sans-serif',
          fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6, marginTop: 10,
        }}>Vela will keep this for your next return.</p>
      </div>
        ...
        {OPTIONS.map(opt => {
          ...
          return (
            <button key={opt.id} onClick={() => toggle(opt.id)} style={{
              ...
              borderRadius: 12,
              border: isSelected ? '1.5px solid rgba(188,140,200,0.6)' : '1.5px solid rgba(255,255,255,0.08)',
              background: isSelected ? 'rgba(188,140,200,0.1)' : 'rgba(255,255,255,0.04)',
              cursor: 'pointer', textAlign: 'left', transition: 'all 0.18s ease',
            }}>
              <div style={{
                width: 20, height: 20, borderRadius: '50%',
                border: isSelected ? '2px solid rgba(188,140,200,0.8)' : '1.5px solid rgba(255,255,255,0.2)',
                background: isSelected ? 'rgba(188,140,200,0.7)' : 'transparent',
              ...
              <div>
                <p style={{
                  ...
                  color: isSelected ? 'rgba(188,140,200,0.9)' : 'var(--text-primary)', transition: 'color 0.18s',
                }}>{opt.label}</p>
                <p style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 28 }}>
        <button onClick={() => onContinue(selected)} disabled={selected.length === 0} style={{
          height: 48, borderRadius: 24,
          background: selected.length > 0 ? 'rgba(130,165,120,0.5)' : 'rgba(255,255,255,0.06)',
        ...
        <button onClick={onSkip} style={{
          height: 40, borderRadius: 20, background: 'transparent', border: 'none',
