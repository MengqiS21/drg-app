...
export default function SwipeableThreadCard({
  session,
  expanded,
  onToggleExpand,
  onDelete,
  ...
  return (
    <div className="swipe-thread-wrap">
      <button
        type="button"
      ...
      >
        <button
          type="button"
          className="sidebar-session-card swipe-thread-card"
          onClick={() => {
        ...
        >
          ...
          {session.type && <p className="sidebar-session-type">{session.type}</p>}
          <p
          ...
          <p className="swipe-thread-hint">Swipe left to delete</p>
        </button>
      </div>
    </div>
  );
